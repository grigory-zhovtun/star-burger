...
- `YANDEX_GEOCODER_APIKEY` — API-ключ для Яндекс Геокодера. Используется для определения координат адресов и расчёта расстояний до ресторанов. Получить ключ можно в [кабинете разработчика Яндекс](https://developer.tech.yandex.ru/).
